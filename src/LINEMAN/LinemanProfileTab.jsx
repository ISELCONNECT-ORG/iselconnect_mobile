...
import { Settings, User, Edit, Save, X, Users } from "lucide-react";
  ...
  const fetchUserData = async () => {
    try {
      ...
      if (user) {
        ...
        const empData = Array.isArray(dbUser?.employees)
          ? dbUser.employees[0]
          : dbUser?.employees;
        const empId = empData?.employee_id_no || "N/A";

        // Search the lineman_teams table to find where this user belongs
        ...
        if (teamsData) {
          const myTeam = teamsData.find((team) => {
            ...
            if (team.team_leader === user.id) return true;

            // Check if they are in the members array/json
            if (Array.isArray(team.team_members)) {
              ...
              return team.team_members.includes(user.id);
            }
          ...
          if (myTeam) {
            teamName = myTeam.team_name || "Assigned";
          }
        }
        ...
        setUserProfile({
          ...
          employeeId: empId,
          teamName: teamName, // Safely assigned team name!
          address: fullAddress,
        ...
        {!isEditing ? (
          <>
            <div className="pt-data-grid page-transition">
              {/* 🌟 MOVED: Assigned Team Display Row is now on top */}
              <div
              ...
              {/* 🌟 MOVED: Employee ID is now below the team */}
